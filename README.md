...
Предназначен для отображения модальных окон.

Поля:
- modalContainer: HTMLElement: контейнер, в котором отображаются модальные окна.
- currentContent: ModalContent: текущий отображаемый контент (экземпляр класса представления конкретного контента)
